...
def generate_daily_summary(trade_date: date) -> dict | None:
    ...
    try:
        ...
        summary.profit_factor = profit_factor if profit_factor != float("inf") else 999.0
...
def mine_patterns(trade_date: date) -> list[dict]:
    ...
    try:
        ...
        trades = db.query(PaperTrade).filter(
            PaperTrade.trade_date.between(lookback_start, trade_date),
...
def get_learnings():
    db = SessionLocal()
    try:
        learnings = db.query(StrategyLearning).filter(
            StrategyLearning.is_active == True
        ).order_by(StrategyLearning.confidence.desc()).all()
