...
from shared.models import TradeDecision, PaperTrade, StockAnalysis
...
def execute_market_open_trades():
    ...
    db = SessionLocal()
    try:
        approved = db.query(TradeDecision).filter(
            TradeDecision.trade_date == today,
        ...
        for decision in approved:
            ...
            existing = db.query(PaperTrade).filter(
                PaperTrade.trade_date == today,
                PaperTrade.signal_id == decision.signal_id
            ).first()
            if existing:
                continue
...
def get_open_trades(trade_date):
    ...
    try:
        trades = db.query(PaperTrade).filter(
            PaperTrade.trade_date == d, PaperTrade.status == "OPEN"
        ).all()
        return jsonify([t.to_dict() for t in trades])
    finally:
        db.close()
